  <footer class="text-center"  >
            Copyright &copy;2017-<?php echo date('Y'); ?> <a href="">By Rohail Butt | All Rights Reserved</a>
  </footer>
      </div>    
  ...
  content_css: [
    ...
    <?php 
        ...
        $run=mysqli_query($conn,$media_query);
     ...
     image_list: [
         <?php 
            while($row=mysqli_fetch_array($run)){ 
                $name=$row['image'];
         ?>
    {title: '<?php echo $name; ?>', value: 'media/<?php echo $name; ?>'},
   <?php }?>
  ]
        <?php }?>
};
